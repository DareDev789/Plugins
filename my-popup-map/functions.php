        ...
        <div style="max-height: 38vh; overflow-y: auto; width: 100%;">
            <ul style="list-style-type: none; padding-left: 0;">
                ...
                if ($selected_categories) {
                    ...
                    // Séparer les catégories avec et sans ordre défini
                    $ordered_categories = [];
                    $unordered_categories = [];
                    
                    foreach ($categories as $category) {
                        if (isset($category_orders[$category->term_id]) && $category_orders[$category->term_id] != '') {
                            $ordered_categories[] = $category; // Catégories avec un ordre défini
                        } else {
                            $unordered_categories[] = $category; // Catégories sans ordre défini
                        }
                    }

                    // Trier les catégories par leur ordre défini
                    usort($ordered_categories, function($a, $b) use ($category_orders) {
                        return $category_orders[$a->term_id] - $category_orders[$b->term_id];
                    });

                    // Trier les catégories sans ordre défini par ordre alphabétique
...
function my_popup_map_settings_page() {
    // Obtenez uniquement les catégories parents (parent = 0)
    $categories = get_terms(array(
        'taxonomy' => 'product_cat',
        'hide_empty' => false,
        'parent' => 0, 
        'orderby' => 'name', // Tri par ordre alphabétique
        'order' => 'ASC'
    ));
    ...
    // Récupérer tous les numéros déjà attribués
...
function my_popup_map_menu() {
    add_menu_page(
        'Personnaliser les Catégories', // Titre de la page
        'Catégories Popup',             // Texte du menu
        'manage_options',               // Capacité requise
        'my-popup-map-settings',        // Slug de la page
        'my_popup_map_settings_page',   // Fonction qui affiche la page
        'dashicons-list-view',          // Icône du menu
        60                              // Position du menu
    );
}
